    ...
    async function sendMessage() {
        ...
        try {
            ...
            if (data.finished) {
                ...
                const actionHtml = `
                <div id="chat-action" class="mt-3 d-flex gap-2">

                    <button id="consult-again" class="btn btn-outline-primary">
                        🩺 Konsultasi Lagi
                    </button>
                    ...
                    <button id="finish-chat" class="btn btn-danger">
                        ❌ Selesai
                    </button>
                ...
                chatBox.insertAdjacentHTML("beforeend", actionHtml);
                document.getElementById("consult-again").onclick = async function () {

                    await fetch("/api/reset-chat", {
                        method: "POST"
                    });

                    document.getElementById("chat-action").remove();

                    chatBox.insertAdjacentHTML(
                        "beforeend",
                        `
                        <div class="mb-3">
                            <div class="p-3 bg-white border rounded d-inline-block shadow-sm">
                                <strong>🤖 AI Dokter:</strong><br>
                                Silakan ceritakan keluhan Anda yang lain.
                            </div>
                        </div>
                        `
                    );
                ...
                // Selesai
                document.getElementById("finish-chat").onclick = async function () {
